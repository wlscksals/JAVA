...
class C03GUI extends JFrame implements ActionListener, KeyListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		// area1 시작 
		if(th1 == null) { // th1을 멤버로 구성후 if(th1 == null)을 주고 Thread th1 = new Thread(w1)에 Thread를 빼면 area1을 눌러도 Thread가 반복 생성되지 않는다.
			C03Worker1 w1 = new C03Worker1(this); //this 현재 class(C03GUI)의 주소가 존달된다.
			th1 = new Thread(w1);
			th1.start();
			}
		}
		...
		if (e.getSource() == btn2) {
			...
			if(th1 != null) {
				th1.interrupt(); //싹다 비우고 예외를 만들어비운다. 그걸 catch가 받눈다.
			}
		...
		if (e.getSource() == btn3) {
			...
			// area2 시작 
			if(th2 == null) { // th1을 멤버로 구성후 if(th1 == null)을 주고 Thread th1 = new Thread(w1)에 Thread를 빼면 area1을 눌러도 Thread가 반복 생성되지 않는다.
				C03Worker2 w2 = new C03Worker2(this); //this 현재 class(C03GUI)의 주소가 존달된다.
				th2 = new Thread(w2);
				th2.start();
				}
			}
		...
		if (e.getSource() == btn4) {
			...
			if(th2 != null) {
				th2.interrupt(); //싹다 비우고 예외를 만들어비운다. 그걸 catch가 받눈다.
			}
