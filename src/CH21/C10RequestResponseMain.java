...
public class C10RequestResponseMain {
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		//Http Request 구조화
		HttpRequest httpReqest = HttpRequest.newBuilder()
				.uri(URI.create(url))	//어느 위치로
				.GET()	//어떤 방식으로
				.build();
		
		//Http Request 객체로 요청 & 응답 확인
		HttpClient httpClient = HttpClient.newHttpClient();
		HttpResponse<String> response = httpClient.send(httpReqest,HttpResponse.BodyHandlers.ofString()); // (httpReqest: 바디 부분을 어떤 식으로 , 문자열로 해석)
		...
		System.out.println("status : " + responseData.getTotal());
